...
def extract_chain_ids(pdb_file):
    ...
    return chains
#TODO 
def remove_non_beta_sheet_ends(pdb_file):
    pass
...
def ran_preprocess_pdb(job):
    return job.isfile('0_preprocess/solvated_ions.gro')
# @Project.label
...
def preprocess_pdb(job):
    try:
        ...
        shutil.copy(f"{job.sp.data_dir}/{job.sp.pdbID}.pdb", job.path + '/0_preprocess')
        ...
        os.system(f'cp {job.sp.data_dir}/{job.sp.pdbID}.pdb .')
        rename_amino_acid(f'{job.sp.pdbID}.pdb', renamed_pdb)
        # renumber_residues(renamed_pdb, renumbered_pdb)
        remove_ligands_water_ions(renamed_pdb, cleaned_pdb)
...
def preprocess_pull(job):
    ...
    # Remove positional restraints for all chains except pull_group2
    for chain in chains:
        if chain != list(chains)[job.sp.pull_group2]:
...
def run_analysis(job):
    ...
    if not os.path.exists(job.path + f'/0_preprocess/{job.sp.pdbID}.pdb'):
    ...
    try:
        ...
        cross_sectional_area = calculate_cross_sectional_area(fibril_axis, job.path+ f'/0_preprocess/{job.sp.pdbID}.pdb')
