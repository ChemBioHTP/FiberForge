#!/usr/bin/env python

import os
import numpy as np

bias = np.loadtxt("COLVAR",usecols=2)
bmax = max(bias)
kbt=2.494339

colvar = np.loadtxt("COLVAR")
dw = []
for line in colvar:
    a = line[1], np.exp((line[2]-bmax)/kbt)
    dw.append(a)

np.savetxt('dist.weight',dw,fmt='%1.9f')

# calculate the free-energy landscape and the associated errors for different block-sizes (i) 
# then calculate the average error in each bin
# python3 do_block_fes.py CV-bias-file No-CVs Min-val Max-val No-bins Kbt block-size

errors = []

for i in range(1,1000,10):
    os.system('python3 do_block_fes.py dist.weight 1 0.3 4.5 201 2.494339 {i}'.format(i=i))

    file = "fes.{i}.dat".format(i=i)
    err = np.genfromtxt(file,usecols=2,skip_footer=1)
    average = i, sum(err)/len(err)
    errors.append(average)

np.savetxt('errors.block',errors,fmt='%1.9f')

