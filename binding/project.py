...
def create_job_files(job):
    ...
    with open(job.path + '/4_sys_eq/heat.in', 'w') as f:
        data = [
            ...
            f"   ntc=2,restraintmask=':1-{len(job.sp.sequences[0]) + len(job.sp.sequences[1])}',",
            '   restraint_wt=10.0,',
            '&end',
            '',
    ...
    with open(job.path + '/5_meta/plumed.dat', 'w') as f:
        data = [
            ...
            '',
            '# define CV',
            f'ab: ANTIBETARMSD RESIDUES=1-{len(job.sp.sequences[0]) + len(job.sp.sequences[1])} STRANDS_CUTOFF=1',
            '',
